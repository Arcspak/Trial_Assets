...
namespace Arcspark.CommonUtils
{
    ...
    public class DownloadRequest
    {
            ...
            try
            {
                switch (type)
                {
                    case DownloadType.FILE:
                        downloadedFileBytes = uwr.downloadHandler.data;
                        downloadedFileText = uwr.downloadHandler.text;
                        loadSuccess(downloadedFileBytes, downloadedFileText);
                        break;
                    case DownloadType.ASSET_BUNDLE:
                        downloadedAssetBundle = DownloadHandlerAssetBundle.GetContent(uwr);
                        downloadedAssetBundleBytes = uwr.downloadHandler.data;
                        loadSuccess(downloadedAssetBundleBytes, downloadedAssetBundle);
                        break;
                    default:
        ...
        public byte[] DownloadedBytes
        {
            get
            {
                ...
                if (type == DownloadType.ASSET_BUNDLE &&
                    DownloadedAssetBundleBytes != null)
                    return downloadedAssetBundleBytes;
        ...
        {
            get => downloadedAssetBundleBytes;
        }
        ...
        {
            get => downloadedAssetBundle;
        }

        private byte[] downloadedFileBytes;
        private string downloadedFileText;
        private byte[] downloadedAssetBundleBytes;
        private AssetBundle downloadedAssetBundle;
